...
import { Heart, MessageCircle, UserPlus, ArrowLeft, Check, Bell, Settings, Phone } from "lucide-react";
...
export const NotificationsPage = () => {
  ...
  const handleAction = async (senderId, action) => {
    try {
      const res = await api.post(`/user/follow-request/${action}`, { senderId });
      if (res.data.success) {
  ...
  const handleSaveSettings = async (newSettings) => {
    ...
    try {
      const res = await api.put("/notification/settings", { settings: newSettings });
  ...
  return (
    <div className="min-h-screen bg-bg text-text p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`py-1.5 px-4 rounded-xl text-xs font-bold transition ${
              activeTab === "activity" ? "bg-rose-600 text-text shadow" : "text-text-secondary hover:text-text bg-surface"
            }`}
          >
            Activity Feed
          </button>
          <button
            ...
            className={`py-1.5 px-4 rounded-xl text-xs font-bold transition ${
              activeTab === "settings" ? "bg-rose-600 text-text shadow" : "text-text-secondary hover:text-text bg-surface"
            }`}
          >
            Preferences
          </button>
      ...
      {activeTab === "activity" ? (
        <div className="space-y-6">
          ...
          {requests.length > 0 && (
            <div className="bg-surface border border-rose-500/20 rounded-3xl p-5 space-y-4 shadow-xl text-left">
                  ...
                  <div key={reqUser._id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                    <div className="flex items-center gap-3 min-w-0">
                      <img
                        ...
                        alt=""
                        className="w-10 h-10 rounded-full object-cover border border-border cursor-pointer shrink-0"
                        onClick={() => navigate(`/profile/${reqUser.userName}`)}
                      />
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow transition cursor-pointer"
                      >
                        Confirm
                      </button>
                      <button
                        onClick={() => handleAction(reqUser._id, "decline")}
                        className="px-3.5 py-1.5 bg-surface-inset hover:bg-surface border border-border text-text-secondary font-bold text-xs rounded-xl transition cursor-pointer"
                      >
                        Delete
                      </button>
                    </div>
            ...
            <>
              ...
              {todayNotifs.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-bold text-rose-400 uppercase tracking-wider">Today</h3>
                  <div className="space-y-2">
                    {todayNotifs.map((item) => (
                      <NotificationItem key={item._id} notif={item} navigate={navigate} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {hasMore && (
                <button
                  ...
                  className="w-full py-2.5 bg-surface hover:bg-surface-hover border border-border text-text text-xs font-bold rounded-2xl transition cursor-pointer"
                >
...
const NotificationItem = ({ notif, navigate }) => {
  const sender = notif.sender;
  if (!sender) return null;

  return (
    <div className="flex items-center justify-between p-3.5 bg-surface/70 hover:bg-surface border border-border/80 rounded-2xl transition">
      <div className="flex items-center gap-3.5 flex-1 min-w-0">
        ...
        <div className="text-xs truncate space-y-0.5">
          <p className="text-text font-medium truncate">
            <span className="font-bold cursor-pointer hover:underline" onClick={() => navigate(`/profile/${sender.userName}`)}>
              @{sender.userName}
            </span>{" "}
      ...
      {notif.type === "follow" ? (
        <FollowButton
          ...
          tailwind="px-4 py-1.5 bg-rose-600 text-text font-bold text-xs rounded-full shadow shrink-0 ml-3"
        />
      ) : notif.type === "call" ? (
        ...
        <img src={notif.post.media.url} alt="" className="w-10 h-10 rounded-xl object-cover border border-border shrink-0 ml-3" />
      ) : null}
    </div>
  );
};
